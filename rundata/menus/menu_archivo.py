...
class Menu_Archivo(Menu):
    def  __init__(self,parent,x,y):
        ...
        cascadas = {
            'exportar':[
                {n:'Colisiones', w:lambda: FileDiag({'scr': 'Guardar', 'tipo': 'G', 'cmd': Sys.GuardarMapaDeColisiones},
                                                    filetypes=['*.png'], carpeta_actual=Sys.fdExport)},
                {n:'Mapa', w:lambda: FileDiag({'scr': 'Guardar', 'tipo': 'G', 'cmd': Sys.exportarMapa},
                                              filetypes=['*.json'], carpeta_actual=Sys.fdExport)}]}
        opciones = [
            {n:'Nuevo',c:lambda:CuadroMapa('Nuevo Mapa'),"icon":Sys.iconos['nuevo'],k:'Ctrl+N'},
            {n:'Abrir', w:lambda: FileDiag({'scr': 'Abrir', 'tipo': 'A', 'cmd': Sys.abrirProyecto}, filetypes=['.json'],
                                           carpeta_actual=Sys.fdProyectos), "icon":Sys.iconos['abrir'], k: 'Ctrl+A'},
            {n:'Guardar',c:self.Guardar,"icon":Sys.iconos['guardar'],k:'Ctrl+S'},
            {n:'Guardar como', w:lambda: FileDiag({'scr': 'Guardar', 'tipo': 'Gc', 'cmd': Sys.guardarProyecto},
                                                  carpeta_actual=Sys.fdProyectos), k: 'Ctrl+Alt+S'},
